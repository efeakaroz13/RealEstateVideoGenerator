...
class Anton:
    ...
    def getData(self, city, state, limit=4):
        ...
        around_median_homes_for_sale = data["result"]["results"][
            "around_median_homes_for_sale"
        ]["properties"]
        new_listings_for_sale = data["result"]["results"]["new_listings_for_sale"][
            "properties"
        ]
        open_houses_for_sale = data["result"]["results"]["open_houses_for_sale"][
            "properties"
        ]
        affordable_homes_for_sale = data["result"]["results"][
            "affordable_homes_for_sale"
        ]["properties"]
        luxury_homes_for_sale = data["result"]["results"]["luxury_homes_for_sale"][
            "properties"
        ]
        output = []
    ...
    def getZillowData(self, city, state):
        page1 = json.loads(
            requests.get(
                f"https://www.zillowstatic.com/autocomplete/v3/suggestions?q={city},{state}&resultTypes=allRegion,allAddress&clientId=static-search-page"
            ).content
        )
        regionId = page1["results"][0]["metaData"]["regionId"]

        searchData = {
            "mapBounds": {
                "north": 42.467600080301146,
                "east": -70.8107322998047,
                "south": 42.158884482813384,
            ...
            "isListVisible": True,
            "mapZoom": 8,
            "regionSelection": [{"regionId": regionId}],
            "pagination": {},
        }
        ...
        page = requests.get(
            f"https://www.zillow.com/search/GetSearchPageState.htm?searchQueryState={searchData}&wants={wantsdata}",
            headers={"User-Agent": random.choice(agents).replace("\n", "")},
        )
    ...
    def videoGenRealtor(self, data):
        ...
        Address = data["location"]["address"]["line"]+ ", "+ data["location"]["address"]["city"]+ ","+ data["location"]["address"]["state_code"]+ " "+ data["location"]["address"]["postal_code"]
        ...
        for i in images:
            ...
            editor = DetailsEditor(
                ...
                baths=baths,
                sqfeet=sqfeet,
                lotsqfeet=data["description"]["lot_sqft"],
                list_date=list_date,
                image_name=i,
            )
            editor.save(fname)
        ...
        if len(allSounds) > 0:
            ...
            audioclip = AudioFileClip(f"ncs/{audioSelected}").subclip(
                0, len(editedImages) * 5
            )

            new_audioclip = CompositeAudioClip([audioclip])
            video.audio = new_audioclip
        videoname = f"static/{self.FileNameCreate(12)}.mp4"
        video.write_videofile(videoname, fps=30,codec="mpeg4")
        for e in editedImages:
            os.system(f"rm '{e}'")
        return videoname
    ...
    def videoGenZillow(self, data):
        ...
        for e in editedImages:
            mclip = ImageClip(e)
            mclip = mclip.set_duration(5)
            mclip = mclip.resize(height=1080)
            clips.append(mclip)
